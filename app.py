...
DESKTOP_MODEL_PATH = os.path.join(BASE_DIR, 'best_dental_model.h5')
desktop_model = None
if os.path.exists(DESKTOP_MODEL_PATH):
    try:
        ...
        print(f"Desktop model input shape: {desktop_model.input_shape}")
    except Exception as e:
        print(f"Error loading desktop model: {str(e)}")

# Load mobile model (optimized for mobile devices)
MOBILE_MODEL_PATH = os.path.join(BASE_DIR, 'dental_model.tflite')
...
if os.path.exists(MOBILE_MODEL_PATH):
    try:
        ...
        print(f"Mobile model input shape: {mobile_input_details[0]['shape']}")
    except Exception as e:
        print(f"Error loading mobile model: {str(e)}")

# Load class names
CLASS_NAMES_PATH = os.path.join(BASE_DIR, 'class_names.json')
class_names = ['cavity', 'gingivitis', 'tooth decay', 'healthy teeth']  # Default class names
if os.path.exists(CLASS_NAMES_PATH):
    ...
    print(f"Class names file not found at {CLASS_NAMES_PATH}, using defaults")

print(f"Models loaded in {time.time() - start_time:.2f} seconds")

# Define target size based on model requirements
if desktop_model:
    TARGET_SIZE = (desktop_model.input_shape[1], desktop_model.input_shape[2])  # (height, width)
elif mobile_interpreter:
    TARGET_SIZE = (mobile_input_details[0]['shape'][1], mobile_input_details[0]['shape'][2])
else:
    TARGET_SIZE = (224, 224)  # Default fallback size
...
def predict_image(img_path, is_mobile=False):
    """Predict dental condition from image using OpenCV"""
    try:
        ...
        if is_mobile and mobile_interpreter:
            ...
            if img_array.shape != tuple(mobile_input_details[0]['shape']):
                img_array = np.resize(img_array, mobile_input_details[0]['shape'])
        ...
        while (time.time() - start_time) < max_duration:
            ...
            if frame_index % frame_skip == 0:
                try:
                    ...
                    processed_frame = preprocess_frame(cv2.resize(frame.copy(), TARGET_SIZE))
